...
const taskbox = [pending, progress, completed];
...
Board.getAllTasks().forEach((tasks, index) => {
    tasks.forEach(task => {
...
addForm.forEach(form => {
    form.addEventListener("submit", event => {
        ...
        if(form.task.value){
            const task = Board.addTask(form.submit.dataset.id, form.task.value.trim());
...
taskbox.forEach(column => {
    column.addEventListener("click", event => {
        ...
        if(event.target.classList.contains("delete")){
            ...
            Board.delTask(event.target.dataset.id);
        }
